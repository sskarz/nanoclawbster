...
│  │    • mcp__nanoclawbster__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── nanoclawbster.log               # Host stdout
│   └── nanoclawbster.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.nanoclawbster.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'nanoclawbster-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your nanoclawbster installation
...
NanoClawbster uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: nanoclawbster (scheduler)
   │
   ▼
...
NanoClawbster has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__nanoclawbster__schedule_task]
        {
...
### NanoClawbster MCP (built-in)

The `nanoclawbster` MCP server is created dynamically per agent call with the current group's context.
...
NanoClawbster runs as a single macOS launchd service.

### Startup Sequence

When NanoClawbster starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NanoClawbster containers from previous runs
...
### Service: com.nanoclawbster

**launchd/com.nanoclawbster.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/nanoclawbster.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/nanoclawbster.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.nanoclawbster.plist ~/Library/LaunchAgents/
...
# Check status
launchctl list | grep nanoclawbster

# View logs
tail -f logs/nanoclawbster.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nanoclawbster` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; NanoClawbster auto-starts container runtime but may fail |
...
- `logs/nanoclawbster.log` - stdout
- `logs/nanoclawbster.error.log` - stderr
