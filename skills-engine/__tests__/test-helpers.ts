...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nanoclawbster-test-'));
}

export function setupNanoclawDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.nanoclawbster', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.nanoclawbster', 'backup'), { recursive: true });
}
