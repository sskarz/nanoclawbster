...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    ...
    const patchPath = path.join(tmpDir, '.nanoclawbster', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.nanoclawbster', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
      ...
      custom_modifications: [
        {
          ...
          patch_file: '.nanoclawbster/custom/001-tweaked-config.patch',
        },
      ],
    });
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.nanoclawbster', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.nanoclawbster', 'base', 'src', 'config.ts'),
      'utf-8',
    );
