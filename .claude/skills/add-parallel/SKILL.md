...
- **Non-blocking Design** - Uses NanoClawbster scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NanoClawbster already set up and running
...
const mcpServers: Record<string, any> = {
  nanoclawbster: ipcMcp
};
```

Add Parallel AI MCP servers after the nanoclawbster server:
```typescript
const mcpServers: Record<string, any> = {
  nanoclawbster: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__nanoclawbster__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__nanoclawbster__send_message
   4. Use mcp__nanoclawbster__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo nanoclawbster-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep nanoclawbster  # macOS
...
```bash
tail -20 logs/nanoclawbster.log
```
...
- Check task runs: `tail -f logs/nanoclawbster.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.nanoclawbster` (macOS) or `systemctl --user restart nanoclawbster` (Linux)
