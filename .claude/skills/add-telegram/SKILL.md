...
This skill adds Telegram support to NanoClawbster using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.nanoclawbster/state.yaml`. If `telegram` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep nanoclawbster` (macOS) or `systemctl --user status nanoclawbster` (Linux)
...
- Check bot is started: `tail -f logs/nanoclawbster.log`
...
8. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.nanoclawbster` (macOS) or `npm run build && systemctl --user restart nanoclawbster` (Linux)
