...
# NanoClawbster Container Debugging
...
|-----|----------|---------|
| **Main app logs** | `logs/nanoclawbster.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/nanoclawbster.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash nanoclawbster-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash nanoclawbster-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash nanoclawbster-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash nanoclawbster-agent:latest
```
...
# Also clear the session ID from NanoClawbster's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/nanoclawbster.log | tail -5
...
```bash
echo "=== Checking NanoClawbster Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo nanoclawbster-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/nanoclawbster.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
