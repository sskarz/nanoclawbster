...
This skill switches NanoClawbster's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
Read `.nanoclawbster/state.yaml`. If `convert-to-apple-container` is in `applied_skills`, skip to Phase 3 (Verify). The code changes are already in place.
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo nanoclawbster-agent:latest "Container OK"
```
