...
This skill adds Discord support to NanoClawbster using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.nanoclawbster/state.yaml`. If `discord` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
4. Service is running: `launchctl list | grep nanoclawbster`
...
- @mention translation (Discord `<@botId>` → NanoClawbster trigger format)
